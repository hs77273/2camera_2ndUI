    ...
    def run(self):
        try:
            ...
            while True:
                ...
                frame1 = cv2.resize(frame1, (680, 480))
                frame2 = cv2.resize(frame2, (680, 480))

                detected_classes_list = YOLO_OBJECT.process_objects(frame1)
                behaviour_dict = YOLO_GESTURE.process_behaviour(frame2)
                seatobj_dict = SEAT_OBJ.process_seatobjects(frame2)
                ...
                cabin_frame = draw_seats(frame2,CONFIG.seat_coordinates)
